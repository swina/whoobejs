<template>
    <div 
        ...
        v-if="doc"
        :key="doc.id"
        :class="'content-slider max-w-screen overflow-x-hidden h-full flex flex-no-wrap block ' + classe(doc.css)" :style="doc.style + ' ' +  background(doc)" :ref="doc.id">
        
            <template v-for="(block,i) in doc.blocks">
                  <blocks-block-preview-container
                  :class="'slider_' + doc.id + ' slide flex-none top-0 left-0 right-0 bottom-0 w-full slide_' + i"
            ...
            <div v-if="doc.controls.dots.enable" class="absolute bottom-0 left-0 text-center flex-row justify-center items-center p-1 w-full cursor-pointer">
            ...
            <div v-if="doc.controls.navigation.enable" class="cursor-pointer">
            ...
            <div v-if="doc.controls.buttons" :class="'slider-buttons absolute left-0 text-center hidden md:flex md:flex-row border-b justify-left items-center my-2 w-full cursor-pointer ' + doc.controls.buttons_position">
...
<script>
import MokaElement from './block.element.component.vue'
import MokaPreviewContainer from './block.preview.single.container.vue'
        ...
        goTo(n){
          ...
          let tl = gsap.timeline()
            if ( document.querySelector('.slider_' + this.doc.id ) ){
              ...
              tl.to ( '.slider_' + this.doc.id , { opacity:0 , duration: .4 } ),
              tl.to ( '.slider_' + this.doc.id , { xPercent: -this.index*100 , opacity:.5 , duration: .5 } ),
              tl.to ( '.slider_' + this.doc.id , { opacity:1 , duration: .5 })
            }
        ...
        next(n){
            ...
            if ( document.querySelector('.slider_' + this.doc.id ) ){
              this.index === 0 && this.doc.controls.delay ?
                tl.to ( '.slider_' + this.doc.id , { xPercent: 50 , opacity:1 , duration: 0 } ) : null
              
              tl.to ( '.slider_' + this.doc.id , { xPercent: -this.index*100 , opacity:1 , duration: 1.5 } )
              
              this.index === (this.doc.blocks.length -1) && this.doc.controls.delay ?
                tl.to ( '.slider_' + this.doc.id , { xPercent: -this.index*100 , opacity:1 , duration:.3 } ) : null
            }
        },
        classe(css){
          if ( !css ) return
