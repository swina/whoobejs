<template>
    ...
    <nav v-if="el.element === 'menu'" :class="menu_responsive(el) + ' z-top ' + el.css.align +  background(el) + ' ' + el.css.css"> 
        <div menu_item v-for="(item,i) in el.items" :class="el.css.items  + ' cursor-pointer relative '" :key="el.id + '_' + i">
        
            <a :class="el.css.items" v-if="hasNoSubmenu(item)" :href="item.link" @mouseover="menuover=null">
            ...
            <div :ref="'submenu_'+i" submenu v-if="item.submenu && item.submenu.length" :class="isOver(i) + ' ' + el.css.submenu + ' fixed flex flex-col z-highest'" @mouseleave="el.css.submenu_behavior?menuover=null:menuover=null">
...
export default {
    ...
    methods:{
        ...
        isOver(i){
            return i < 0 ? 'opacity-0 invisible h-0' : this.menuover === i ? 'opacity-100 visible h-auto' : 'opacity-0 invisible h-0'
        },
