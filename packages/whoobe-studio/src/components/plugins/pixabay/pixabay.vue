    ...
    <div class="px-8 w-full theme-dark" v-if="$attrs.apikey">
        <div class="flex flex-row items-center w-full mb-4 p-2 relative">
            ...
            <template v-if="!preview">
                <icon v-if="images && page > 1" @click="page--,pixabayQry(search)" class="text-5xl rounded-full ml-6 center-vertical left-0" name="chevron_left"/>
                <icon v-if="images" @click="page++,pixabayQry(search)" class="text-5xl rounded-full mr-1 center-vertical right-0" name="chevron_right"/>
                <icon name="close" @click="$emit('close')" class="absolute right-0 mr-4 text-3xl"/>
            </template>
        </div>
            ...
            <section v-for="(image,i) in images.hits">
                <div class="flex flex-col mb-2 mx-1" title="click to preview" :data-tool-tip="image.downloads > 1000 ? 'Downloads: ' +  parseInt(image.downloads/1000) + 'K' : 'Downloads ' + image.downloads">
                    ...
                    <div class="text-xs text-gray-400 flex flex-row items-center">
                        <icon name="file_download"/> {{ image.downloads > 1000 ? parseInt(image.downloads/1000) + 'K' : image.downloads }}
                        <icon class="ml-4" name="favorite"/> {{ image.favorites }}
                    </div>
                </div>
            </section>
        ...
        <modal-fullscreen
            ...
            <div slot="content" class="flex flex-col items-end justify-start relative transition-container duration-700 h-90">
                ...
                <!-- <video v-if="type==='video'" controls>
                    <source :src="preview.videos.medium.url"/>
                </video> -->
                ...
                <icon v-if="type!='video' && preview && images && index > 0" @click="preview=null,$loading(true),index>0?index--:index=0,preview=images.hits[index]" class="text-5xl rounded-full p-1 m-2 center-vertical left-0 z-highest" name="chevron_left"/>
                <icon v-if="type!='video' && preview && images && index < 19" @click="preview=null,$loading(true),index<20?index++:index=0,preview=images.hits[index]" class="text-5xl rounded-full p-1 m-2 center-vertical right-0 z-highest" :name="preview?'chevron_right':'menu'"/>
    ...
    methods:{
        ...
        async pixabayQry(search){
            ...
            this.type === 'video' ?
                url = 'videos' : url = ''
            axios.get ( 
                'https://pixabay.com/api/' + url + '?key=' + this.$attrs.apikey + 
                '&q=' + search + 
                usr +
                imgType + 
