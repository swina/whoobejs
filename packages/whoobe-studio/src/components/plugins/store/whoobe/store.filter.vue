<template>
    <div class="w-full whoobe-store-filter">
        ...
        <select v-model="sortBy" v-if="settings.general.display.sort.enabled" :class="settings.general.display.sort.css" class="whoobe-store-sort">
...
export default {
    ...
    watch:{
        sortBy(v){
            console.log ( v )
            v ? this.$emit('sort',v) : this.$emit('reset')
        }
    },
    methods: {
        productSearch ( e ){
